...
const Index: React.FC = () => {
  ...
  const handleSubmit = useCallback(
    (name: string, description: string, factors: AssessmentFactor[]) => {
      const pos = calculatePosition(factors);
      if (selectedId && projects.find((p) => p.id === selectedId)) {
        // Update existing project
        setProjects((prev) =>
          prev.map((p) => (p.id === selectedId ? { ...p, name, description, x: pos.x, y: pos.y, factors } : p)),
        );
      } else {
  ...
  const handleManualDrop = useCallback(
    (x: number, y: number) => {
      ...
      if (userProject) {
        setProjects((prev) => prev.map((p) => (p.isUserProject ? { ...p, x, y } : p)));
      }
    },
    [projects],
  );

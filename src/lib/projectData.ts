...
export const defaultFactors: AssessmentFactor[] = [
  ...
  { id: "task_count", label: "Antal aktiviteter", description: "Hur många aktiviteter innehåller projektplanen?", axis: "x", weight: 0.30, value: 50 },
  { id: "code_wbs", label: "Kodmängd & WBS-struktur", description: "Hur stor kodmängd har planen och hur många nivåer är den nedbruten i (WBS)?", axis: "x", weight: 0.15, value: 50 },
...
export const demoProjects: Project[] = [
  {
    ...
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 85, code_wbs: 65, schedule_detail: 85, resource_count: 60, resource_level: 75, risk_detail: 60, historical_data: 65, kpi_tracking: 60, data_integration: 75, reporting_freq: 90 };
  ...
  },
  {
    ...
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 90, code_wbs: 50, schedule_detail: 60, resource_count: 80, resource_level: 90, risk_detail: 60, historical_data: 70, kpi_tracking: 90, data_integration: 75, reporting_freq: 70 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-27",
  },
  {
    ...
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 65, code_wbs: 35, schedule_detail: 50, resource_count: 75, resource_level: 70, risk_detail: 50, historical_data: 55, kpi_tracking: 45, data_integration: 50, reporting_freq: 50 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-27",
  },
  {
    ...
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 95, code_wbs: 75, schedule_detail: 80, resource_count: 50, resource_level: 55, risk_detail: 70, historical_data: 60, kpi_tracking: 75, data_integration: 55, reporting_freq: 70 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-27",
  },
  {
    ...
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 30, code_wbs: 20, schedule_detail: 25, resource_count: 10, resource_level: 10, risk_detail: 15, historical_data: 15, kpi_tracking: 10, data_integration: 10, reporting_freq: 30 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-27",
  },
  {
    ...
    color: "hsl(var(--chart-1))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 35, code_wbs: 15, schedule_detail: 35, resource_count: 25, resource_level: 30, risk_detail: 30, historical_data: 25, kpi_tracking: 35, data_integration: 20, reporting_freq: 45 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-27",
  },
  {
    ...
    color: "hsl(var(--chart-2))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 10, code_wbs: 10, schedule_detail: 15, resource_count: 10, resource_level: 10, risk_detail: 10, historical_data: 10, kpi_tracking: 10, data_integration: 10, reporting_freq: 25 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-27",
  },
  {
    ...
    color: "hsl(var(--chart-3))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 45, code_wbs: 30, schedule_detail: 55, resource_count: 40, resource_level: 65, risk_detail: 50, historical_data: 50, kpi_tracking: 40, data_integration: 45, reporting_freq: 45 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
  ...
  },
  {
    ...
    color: "hsl(var(--chart-4))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 30, code_wbs: 25, schedule_detail: 30, resource_count: 20, resource_level: 25, risk_detail: 25, historical_data: 25, kpi_tracking: 35, data_integration: 20, reporting_freq: 30 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-28",
  },
  {
    ...
    color: "hsl(var(--chart-5))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 35, code_wbs: 15, schedule_detail: 25, resource_count: 30, resource_level: 35, risk_detail: 35, historical_data: 35, kpi_tracking: 40, data_integration: 40, reporting_freq: 30 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
    createdAt: "2025-10-28",
  },
  {
    ...
    color: "hsl(var(--chart-1))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 60, code_wbs: 75, schedule_detail: 70, resource_count: 45, resource_level: 50, risk_detail: 55, historical_data: 60, kpi_tracking: 85, data_integration: 70, reporting_freq: 60 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
  ...
  },
  {
    ...
    color: "hsl(var(--chart-2))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 70, code_wbs: 60, schedule_detail: 65, resource_count: 45, resource_level: 55, risk_detail: 40, historical_data: 35, kpi_tracking: 35, data_integration: 30, reporting_freq: 55 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
  ...
  },
  {
    ...
    color: "hsl(var(--chart-3))",
    factors: defaultFactors.map(f => {
      const vals: Record<string, number> = { task_count: 85, code_wbs: 75, schedule_detail: 80, resource_count: 55, resource_level: 60, risk_detail: 65, historical_data: 65, kpi_tracking: 80, data_integration: 70, reporting_freq: 70 };
      return { ...f, value: vals[f.id] ?? 50 };
    }),
...
export function describeFactorValue(factorId: string, value: number): string {
  switch (factorId) {
    ...
    case "code_wbs":
      if (value < 25) return "platt struktur med få koder, inga WBS-nivåer";
      if (value < 50) return "enkel kodstruktur, 2–3 nivåer i WBS";
      if (value < 75) return "omfattande kodning, 3–5 nivåer i WBS";
      return "mycket stor kodmängd, 5+ nivåer i WBS";
